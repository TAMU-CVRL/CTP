...
from data.nuscenes_triplet import Triplet_Scene_Nuscenes
...
class CTPTrainer:
    def __init__(self, cfg, nusc=None):
        self.cfg = cfg
        self.nusc = nusc
        self.rank = 0
        self.world_size = 1
    ...
    def _build_dataloader(self):
        ...
        dataset = Triplet_Scene_Nuscenes(
            jsonl_file=self.cfg["Dataset"]["train_data_path"],
            nusc = nusc,
            image_transform=image_transform,
            sparse_to_dense_fn=lambda x: sparse_to_dense(x, target_num=4096)
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", type=str, default="configs/ctp32_ptv3_s_pc.yaml",
                        help="Path to the config file.")
    args = parser.parse_args()
    
    config = load_config(args.config)
    nusc = NuScenes(version='v1.0-trainval', verbose=True, dataroot="/home/ximeng/Dataset/nuscenes_full_v1_0/")
    trainer = CTPTrainer(config, nusc)
    trainer.train()
