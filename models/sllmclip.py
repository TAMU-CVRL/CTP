import torch
import torch.nn as nn
import torch.nn.functional as F
import numpy as np

class sclip_pair(nn.Module):
    def __init__(self, text_encoder, lidar_encoder):
        super().__init__()
        self.text_encoder = text_encoder        
        self.lidar_encoder = lidar_encoder
        self.projector = nn.Linear(1024, 512)
        
        self.logit_scale = nn.Parameter(torch.ones([]) * np.log(1 / 0.07))
        
    def forward(self, lidar, text_ids):
        lidar_features = self.lidar_encoder(lidar)
        lidar_features = self.projector(lidar_features) # project to match text feature dimension
        text_features = self.text_encoder(text_ids)
        text_features = text_features.float()
        
        # normalized features
        text_features = text_features / text_features.norm(dim=-1, keepdim=True)
        lidar_features = lidar_features / lidar_features.norm(dim=-1, keepdim=True)
        
        # cosine similarity as logits
        logit_scale = self.logit_scale.exp()
        logits_per_lidar = logit_scale * lidar_features @ text_features.t()
        logits_per_text = logits_per_lidar.t()
        
        return logits_per_lidar, logits_per_text

    def get_loss(self, logits_per_lidar, logits_per_text):
        batch_size = logits_per_lidar.shape[0]
        labels = torch.arange(batch_size, device=logits_per_lidar.device)
        loss_lidar = F.cross_entropy(logits_per_lidar, labels)
        loss_text = F.cross_entropy(logits_per_text, labels)
        return (loss_lidar + loss_text) / 2
    
    @property
    def param_count(self):
        n = sum(p.numel() for p in self.parameters())
        if n >= 1e9:
            return f"{n/1e9:.2f}B"
        elif n >= 1e6:
            return f"{n/1e6:.2f}M"
        elif n >= 1e3:
            return f"{n/1e3:.2f}K"
        else:
            return str(n)

class sclip_triplet(nn.Module):
    def __init__(self, text_encoder, image_encoder, lidar_encoder, alpha=0.5):
        super().__init__()
        self.text_encoder = text_encoder
        self.image_encoder = image_encoder      
        self.lidar_encoder = lidar_encoder
        self.projector = nn.Linear(1024, 512)
        self.alpha = alpha # weight for image and text features

        self.logit_scale = nn.Parameter(torch.ones([]) * np.log(1 / 0.07))
        
    def forward(self, text_ids, image, lidar):
        lidar_features = self.lidar_encoder(lidar)
        lidar_features = self.projector(lidar_features) # project to match text feature dimension
        image_features = self.image_encoder(image)
        text_features = self.text_encoder(text_ids)
        text_features = text_features.float()
        
        # normalized features
        image_text_features = self.alpha * text_features + (1 - self.alpha) * image_features
        image_text_features = image_text_features / (image_text_features.norm(dim=-1, keepdim=True) + 1e-6)
        lidar_features = lidar_features / (lidar_features.norm(dim=-1, keepdim=True) + 1e-6)
        
        # cosine similarity as logits
        logit_scale = self.logit_scale.exp()
        logits_per_lidar = logit_scale * lidar_features @ image_text_features.t()
        logits_per_image_text = logits_per_lidar.t()
        
        return logits_per_lidar, logits_per_image_text

    def get_loss(self, logits_per_lidar, logits_per_image_text):
        batch_size = logits_per_lidar.shape[0]
        labels = torch.arange(batch_size, device=logits_per_lidar.device)
        loss_lidar = F.cross_entropy(logits_per_lidar, labels)
        loss_image_text = F.cross_entropy(logits_per_image_text, labels)
        return (loss_lidar + loss_image_text) / 2
    
    @property
    def param_count(self):
        n = sum(p.numel() for p in self.parameters())
        if n >= 1e9:
            return f"{n/1e9:.2f}B"
        elif n >= 1e6:
            return f"{n/1e6:.2f}M"
        elif n >= 1e3:
            return f"{n/1e3:.2f}K"
        else:
            return str(n)